...
ServerSetup::ServerSetup(ClanBomberApplication* _app, Server* _server, Client* _client)
{
    ...
    cb_client=_client;
    for(int i=0;i<8;i++) {
        GameConfig::bomber[i].disable();
        GameConfig::bomber[i].set_client_ip(cb_server->get_ip());
        GameConfig::bomber[i].set_server_bomber(true);
        GameConfig::bomber[i].set_highlight_maptile(false);
        GameConfig::bomber[i].set_config_index(i);
        unselected_players.push_back(&GameConfig::bomber[i]);
    }
    GameConfig::save();
    Chat::hide();
    exit_setup=false;
...
void ServerSetup::show_player_selected(int index)
{
    ...
    int cl_index=GameConfig::bomber[index].get_client_index();
    if(cl_index>=0) {
    ...
    if(GameConfig::bomber[index].is_local()) {
        if(cl_index>=0) {
	...
	Resources::Font_small()->render(_("* local *"), cg[4]+50,
        ...
        if(GameConfig::bomber[index].is_server_bomber()) {
            if(cl_index>=0) {
	      ///primary->SetColor(primary, cl_col_r, cl_col_g, cl_col_b, 255);
            }    
            else {
    ...
    if(GameConfig::bomber[index].is_local()) {
      ...
      ///primary->DrawString(primary, GameConfig::bomber[index].get_client_ip(), -1, cg[1]-130, 205+index*50, DSTF_CENTER);
      Resources::Font_small()->render(GameConfig::bomber[index].get_client_ip(),
                                      cg[1]-130, 205+index*50,
                                      cbe::FontAlignment_0center);
    }
    if(GameConfig::bomber[index].get_highlight_maptile()) {
    ...
    Resources::Bombers(GameConfig::bomber[index].get_skin())->blit( cg[0]+12, 230+index*50 - Resources::Bombers(GameConfig::bomber[index].get_skin())->getHeight(), 0.7, 0.7, 0);
    if(!fick || server_setup->cur_row!=index) {
      ///primary->SetFont(primary, Resources::Font_small());
      ///primary->SetColor(primary, 0x20, 0x90, 0xF0, 0xFF);
      ///primary->DrawString(primary, GameConfig::bomber[index].get_name(), -1, cg[1]+20, 190+index*50, DSTF_TOPLEFT);
      Resources::Font_small()->render(GameConfig::bomber[index].get_name(),
                                      cg[1]+20, 190+index*50,
                                      cbe::FontAlignment_0topleft);
    }       
    Resources::Playersetup_teams()->blit(cg[2]+5, 172+index*50, 0.7, 0.7, GameConfig::bomber[index].get_team());
    ///primary->SetFont(primary, Resources::Font_small());
    ///primary->SetColor(primary, 255, 255, 255, 255);
    switch(GameConfig::bomber[index].get_controller()) {   
        case Controller::AI:
            Resources::Playersetup_controls()->blit(cg[3]+20, 172+index*50, 0.7, 0.7, 0);
        ...
        case Controller::KEYMAP_3:
            Resources::Playersetup_controls()->blit(cg[3]+20, 172+index*50, 0.7, 0.7, GameConfig::bomber[index].get_controller()-Controller::KEYMAP_1+1);
            break;
        ...
        case Controller::JOYSTICK_7:
            Resources::Playersetup_controls()->blit(cg[3]+20, 172+index*50, 0.7, 0.7, 4);
            break;
    }
}
...
void ServerSetup::exec()
{
	...
	if(SDL_PollEvent(&event)) {
	  ...
	  if(event.type != SDL_KEYDOWN) {
	    continue;
            ...
            if(!GameConfig::bomber[cur_row].is_local() && cur_col!=2) {
                cur_col=2;
            }
            draw();
            ///primary->Flip(primary, NULL, DSFLIP_WAITFORSYNC);
	    CB_Flip();
            ...
            if(ClanBomberApplication::run_server_with_players()) {
                if(selected_players.size() >= ClanBomberApplication::run_server_with_players()) {
                    if(cb_server->get_nr_of_clients()>0 && GameConfig::get_number_of_opponents()>1) {
          ...
          Resources::Menu_clear()->play();
                ...
                if(GameConfig::bomber[cur_row].is_enabled()) {
                    cb_client->send_SERVER_BOMBER_CONFIG(true,
                                                         GameConfig::bomber[cur_row].get_skin(),
                                                         GameConfig::bomber[cur_row].get_team()+78,
                                                         GameConfig::bomber[cur_row].get_controller(),
                                                         cur_row,
                                                         GameConfig::bomber[cur_row].get_name().c_str());
                }
                break;
		///case DIKI_SPACE:
	case SDLK_SPACE:
                if(GameConfig::bomber[cur_row].is_enabled()) {
                    cb_client->send_SERVER_BOMBER_CONFIG(false,
                                                         GameConfig::bomber[cur_row].get_skin(),
                                                         GameConfig::bomber[cur_row].get_team(),
                                                         GameConfig::bomber[cur_row].get_controller(),
                                                         cur_row,
                                                         GameConfig::bomber[cur_row].get_name().c_str());
                }
	...
	case SDLK_o:
		...
		///case DIKI_H:
	case SDLK_h:
                if(GameConfig::bomber[cur_row].is_local()) {
	...
	case SDLK_DOWN:
                ...
                if(!GameConfig::bomber[cur_row].is_local() && GameConfig::bomber[cur_row].is_enabled()) {
                    cur_col=2;
                }
...
void ServerSetup::draw(bool fick)
{
    ...
    if(!Chat::enabled() && server_setup->cb_server->get_nr_of_clients()>0 && GameConfig::get_number_of_opponents()>1) {
...
void ServerSetup::handle_enter()
{
    switch(cur_col) {
        ...
        case 3:
            ...
            while(GameConfig::bomber[cur_row].get_controller()==Controller::AI ||
                  GameConfig::bomber[cur_row].get_controller()==Controller::AI_mass) {
                GameConfig::bomber[cur_row].set_controller(GameConfig::bomber[cur_row].get_controller()+1);
            }
#endif
