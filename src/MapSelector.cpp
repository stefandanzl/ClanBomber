...
MapSelector::MapSelector( ClanBomberApplication *_app )
{
	...
	if (GameConfig::get_start_map() > map->get_map_count()-1) {
		GameConfig::set_start_map( map->get_map_count() -1 );
	}
	current_map = GameConfig::get_start_map();
	map_at_top = std::min( current_map-5, map->get_map_count()-11 );
	if (map_at_top < 0) map_at_top = 0;
	
	current_theme = GameConfig::get_theme();
...
void MapSelector::exec()
{
  ...
  while (1) {
	...
	case SDLK_BACKSPACE:
	case SDLK_ESCAPE:
	  GameConfig::set_start_map( current_map );
	  GameConfig::save();
	  map->save_selection();
	  return;
	  break;
	...
	case SDLK_LEFT:
	  if (current_theme > 0) {
	    current_theme--;
	    Resources::Menu_clear()->play();
	    GameConfig::set_theme( current_theme );
	    map->load( current_map );
	  }
	  break;
	case SDLK_RIGHT:
...
void MapSelector::draw()
{
  ...
  if (map->get_max_players() < GameConfig::get_number_of_players()) {
