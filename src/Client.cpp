...
int Client::unpack_string(std::string* destination, int* source, int bytelen)
{
  char buffer[bytelen];
  int full_ints = bytelen/4;
  int extra_octets = bytelen%4;
  const uint32_t mask = 0xFF;
  for (int i = 0; i < full_ints; i++) {
    buffer[i*4] = source[i] >> 24;
    buffer[i*4 + 1] = source[i] >> 16 & mask;
    buffer[i*4 + 2] = source[i] >> 8 & mask;
    buffer[i*4 + 3] = source[i] & mask;
  }
  const int extra_start = full_ints*4;
  for (int i = 0; i < extra_octets; i++) {
    buffer[extra_start + i] = source[full_ints] >> 24-i*8 & mask;
  }
  destination->assign(buffer, bytelen);
  int int_length = full_ints + (extra_octets ? 1 : 0);
  return int_length;
}
...
{
    for(int i=0;i<8;i++) {
        if(GameConfig::bomber[i].get_client_index()==previous_client_index) {
            if(ClanBomberApplication::is_server()) {
                send_SERVER_BOMBER_CONFIG(true,
                                          GameConfig::bomber[i].get_skin(),
                                          GameConfig::bomber[i].get_team(),
                                          GameConfig::bomber[i].get_controller(),
                                          i,
                                          GameConfig::bomber[i].get_name().c_str());
            }
...
void Client::recv_SERVER_FULL_BOMBER_CONFIG(int* data, int* pos)
{
    ...
    for(int i=0;i<8;i++) {
        ...
        (*pos)+=intiplen;
        GameConfig::bomber[i].set_client_index(index);
        GameConfig::bomber[i].set_skin(skin);
        GameConfig::bomber[i].set_team(team);
        GameConfig::bomber[i].set_server_bomber(server_bomber);
        GameConfig::bomber[i].set_name(cl_name);
        if(server_bomber) {
            GameConfig::bomber[i].set_client_ip(get_server_ip());
        }
        else {
            GameConfig::bomber[i].set_client_ip(ip);
        }
        if(enable && !GameConfig::bomber[i].is_enabled()) {
            if(ClanBomberApplication::is_server()) {
        ...
        else if(!enable && GameConfig::bomber[i].is_enabled()) {
            if(ClanBomberApplication::is_server()) {
        ...
        GameConfig::bomber[i].set_enabled(enable);
        GameConfig::bomber[i].set_local(my_index==index);
        GameConfig::bomber[i].set_controller(controller);
...
void Client::recv_SERVER_CONFIG(int* data, int* pos)
{
    ...
    GameConfig::set_round_time(round_time);
    GameConfig::set_max_skateboards(max_skateboards); 
    GameConfig::set_max_power(max_power);   
    GameConfig::set_max_bombs(max_bombs);
    GameConfig::set_start_bombs(start_bombs);
    GameConfig::set_start_power(start_power);
    GameConfig::set_start_skateboards(start_skateboards);
    GameConfig::set_start_kick(start_kick);
    GameConfig::set_start_glove(start_glove);
    GameConfig::set_bombs(bombs);
    GameConfig::set_power(power);
    GameConfig::set_skateboards(skateboards);
    GameConfig::set_kick(kick);
    GameConfig::set_glove(glove);
    GameConfig::set_joint(joint);
    GameConfig::set_viagra(viagra);
    GameConfig::set_koks(koks); 
    GameConfig::set_bomb_countdown(bomb_countdown);
    GameConfig::set_bomb_delay(bomb_delay);
    GameConfig::set_bomb_speed(bomb_speed);   
    GameConfig::set_random_map_order(random_map);
    GameConfig::set_random_positions(random_pos);
    GameConfig::set_points_to_win(points_to_win);
    GameConfig::save();
    ...
    for(int j=0;j<bomber_to_add;j++) {
		...
		break;
            ...
            if(!found) {
                ...
                Controller* con=Controller::create((Controller::CONTROLLER_TYPE)GameConfig::bomber[number].get_controller());
                Bomber* b=new Bomber(posx, posy, (Bomber::COLOR)col, con, GameConfig::bomber[number].get_name(), team, number, cb_app);
...
void Client::recv_SERVER_UPDATE_MAP(int* data, int* pos)
{
    ...
    int intnamelen=0;
    std::string map_name;
    intnamelen = unpack_string(&map_name, &data[(*pos)], map_name_length);
    (*pos)+=intnamelen;
    if(!ClanBomberApplication::is_server()) {
