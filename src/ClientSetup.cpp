...
void ClientSetup::exec()
{
    ...
    while (1) {
        ...
        if (cb_client->server_started_game() || cb_client->end_game() || is_disconnected()) {
            ...
            while (cur_row<8 && !GameConfig::bomber[cur_row].is_local() && GameConfig::bomber[cur_row].is_enabled()) {
                cur_row++;
            }
            if (cur_row>7) {
                cur_row=7;
                while (cur_row>=0 && !GameConfig::bomber[cur_row].is_local() && GameConfig::bomber[cur_row].is_enabled()) {
                    cur_row--;
                }
                if (cur_row<0) {
                    server_is_full=true;
            ...
            case DIKI_ENTER:
                ...
                if (GameConfig::bomber[cur_row].is_enabled()) {
                    cb_client->send_CLIENT_BOMBER_CONFIG(true,
                                                         ...
                                                         cur_row,
                                                         GameConfig::bomber[cur_row].get_name().get_string());
                }
                GameConfig::save();
                break;
            case DIKI_SPACE:
            ...
            case DIKI_DOWN:
                {
                    ...
                    while (!ok && row_tmp<7) {
                        row_tmp++;
                        if (GameConfig::bomber[row_tmp].is_local() || !GameConfig::bomber[row_tmp].is_enabled()) {
                            ok=true;
                            cur_row=row_tmp;
                        }
                    }
